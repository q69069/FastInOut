...
<script setup>
defineOptions({ name: 'PurchaseReceipts' })
import { ref, computed, onMounted, onActivated, watch, inject } from 'vue'
import { useRoute, useRouter } from 'vue-router'
import { ElMessage, ElMessageBox } from 'element-plus'
import { getPurchaseReceipts, createPurchaseReceipt, updatePurchaseReceipt, getPurchaseReceipt, confirmPurchaseReceipt, reversePurchaseReceipt, getSuppliers, getWarehouses, getEmployees } from '../../api'
...
const form = ref({
  ...
  remark: '',
  discount_amount: 0,
  items: [{ product_id: null, quantity: 1, unit_price: 0, amount: 0, remark: '', production_date: '' }, { product_id: null, quantity: 1, unit_price: 0, amount: 0, remark: '', production_date: '' }, { product_id: null, quantity: 1, unit_price: 0, amount: 0, remark: '', production_date: '' }, { product_id: null, quantity: 1, unit_price: 0, amount: 0, remark: '', production_date: '' }, { product_id: null, quantity: 1, unit_price: 0, amount: 0, remark: '', production_date: '' }]
})
...
const handleSave = async () => {
  ...
  try {
    ...
    let res
    if (form.value.id) {
      res = await updatePurchaseReceipt(form.value.id, data)
    } else {
      res = await createPurchaseReceipt(data)
    }
    ...
    if (res.data?.id) {
      ...
      if (updateCurrentTabPath) {
        updateCurrentTabPath(`/purchase-receipts?id=${res.data.id}`, `采购单 ${res.data.receipt_no}`)
      }
    }
  } catch (e) {
...
const handleAudit = async () => {
  ...
  await confirmPurchaseReceipt(form.value.id)
  form.value.status = 'confirmed'
  form.value.auditor_name = authStore.displayName
  ElMessage.success({ message: '审核成功，库存已更新', duration: 1500 })
}
...
const handleCopy = () => {
  ...
  router.push({ path: '/purchase-receipts', query: { id: 'copy-' + ts } })
}

const handleNew = () => {
  router.push({ path: '/purchase-receipts', query: { id: 'new-' + Date.now() } })
}

const handleReverse = async (reason) => {
  try {
    await reversePurchaseReceipt(form.value.id, { reason })
    form.value.status = 3
    ...
    if (saved) {
      try {
        ...
        while (form.value.items.length < 5) {
          form.value.items.push({ product_id: null, quantity: 1, unit_price: 0, amount: 0, production_date: '', remark: '' })
        }
        return
      } catch (e) {
...
watch(() => route.query.id, (newId) => {
  if (route.path === '/purchase-receipts') loadOrder(newId)
})
