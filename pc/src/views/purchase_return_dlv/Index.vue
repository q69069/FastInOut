    ...
    <el-card class="header-card" style="position:relative">
      ...
      <div class="page-header">
        <div class="page-title">
          <span class="title-text">采购退货</span>
        </div>
        <div class="header-meta">
          <div class="meta-row">
            <span class="meta-item">单据编号：<em class="text-primary">{{ form.return_dlv_no || '（自动生成）' }}</em></span>
...
<script setup>
defineOptions({ name: 'PurchaseReturnDeliveries' })
import { ref, computed, onMounted, onActivated, watch, inject } from 'vue'
import { useRoute, useRouter } from 'vue-router'
import { ElMessage, ElMessageBox } from 'element-plus'
import { getPurchaseReturnDlvs, createPurchaseReturnDlv, updatePurchaseReturnDlv, getPurchaseReturnDlv, warehouseConfirmPurchaseReturnDlv, financeConfirmPurchaseReturnDlv, deletePurchaseReturnDlv, reversePurchaseReturnDlv, getSuppliers, getWarehouses, getEmployees } from '../../api'
...
const statusMap = { pending: '草稿', warehouse_confirmed: '已出库', finance_confirmed: '已结算', settled: '已结算', reversed: '已冲红' }
...
const form = ref({
  ...
  status: '',
  discount_amount: 0,
  items: [{ product_id: null, quantity: 1, unit_price: 0, amount: 0, remark: '' }, { product_id: null, quantity: 1, unit_price: 0, amount: 0, remark: '' }, { product_id: null, quantity: 1, unit_price: 0, amount: 0, remark: '' }, { product_id: null, quantity: 1, unit_price: 0, amount: 0, remark: '' }, { product_id: null, quantity: 1, unit_price: 0, amount: 0, remark: '' }]
})
...
const onProductChange = (index) => {
  ...
  const p = products.value.find(x => x.id === row.product_id)
  if (p) {
    row._basePrice = p.cost_price || p.retail_price || 0
...
const handleSave = async () => {
  ...
  try {
    ...
    const res = form.value.id ? await updatePurchaseReturnDlv(form.value.id, data) : await createPurchaseReturnDlv(data)
    ElMessage.success('保存成功')
    if (res.data?.id) {
      form.value.id = res.data.id
      form.value.return_dlv_no = res.data.return_dlv_no
      form.value.status = res.data.status
      if (updateCurrentTabPath) {
        updateCurrentTabPath(`/purchase-return-deliveries?id=${res.data.id}`, `采购退货 ${form.value.return_dlv_no}`)
      }
    }
  } catch (e) {
...
const handleAudit = async () => {
  ...
  await ElMessageBox.confirm('确认审核该采购退货单？', '审核确认', { type: 'warning' })
  await warehouseConfirmPurchaseReturnDlv(form.value.id)
  form.value.status = 'warehouse_confirmed'
...
const handleCopy = () => {
  ...
  router.push({ path: '/purchase-return-deliveries', query: { id: 'copy-' + ts } })
}

const handleNew = () => {
  router.push({ path: '/purchase-return-deliveries', query: { id: 'new-' + Date.now() } })
}

const handleReverse = async (reason) => {
  try {
    await reversePurchaseReturnDlv(form.value.id, { reason })
    form.value.status = 3
...
const loadOrder = async (id) => {
    ...
    if (saved) {
      try {
        ...
        form.value = {
          ...
          auditor_name: '',
          auditor_id: null,
          wh_confirmed_by: null,
          wh_confirmed_at: null,
          items: (data.items || []).map(i => {
            ...
            return {
              ...
              remark: i.remark || '',
              unit_id: i.unit_id,
              _basePrice: p?.cost_price || p?.retail_price || 0,
  ...
  try {
    const res = await getPurchaseReturnDlv(id)
...
watch(() => route.query.id, (newId) => {
  if (route.path === '/purchase-return-deliveries') loadOrder(newId)
})
