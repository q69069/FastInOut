<template>
  <div>
    ...
    <el-dialog v-model="dialogVisible" title="新增销售退货" width="800px">
...
import { getSalesReturns, createSalesReturn, updateSalesReturn, deleteSalesReturn, confirmSalesReturn, getProducts, getCustomers, getWarehouses } from '../../api'
...
const loadData = async () => {
  const res = await getSalesReturns(query.value)
  list.value = res.data || []
...
const loadReturnItems = async (id) => {
  try {
    ...
    console.error('[SalesReturn] loadReturnItems error:', e)
  }
}
...
const handleSave = async () => {
  ...
  if (form.value.id) {
    await updateSalesReturn(form.value.id, data)
...
const handleConfirm = async (row) => {
  await ElMessageBox.confirm('确认退货？库存将增加！', '提示', { type: 'warning' })
