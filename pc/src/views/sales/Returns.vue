    ...
    <el-card class="header-card" style="position:relative">
      ...
      <div class="page-header">
        ...
        <div class="header-actions">
          ...
          <el-button type="success" @click="handleFulfill" v-if="form.id && form.status === 1">确认退货入库</el-button>
      ...
      <el-table :data="list" border stripe>
        ...
        <el-table-column label="操作" width="250" fixed="right">
          <template #default="{ row }">
            ...
            <el-button link type="success" size="small" v-if="row.status === 1" @click="handleFulfill(row)">确认退货入库</el-button>
          </template>
        </el-table-column>
      </el-table>
...
import { getSalesReturns, createSalesReturn, updateSalesReturn, getSalesReturn, confirmSalesReturn, reverseSalesReturn, fulfillSalesReturn, getCustomers, getWarehouses, getSalesmen, getEmployees } from '../../api'
...
const detailFields = computed(() => {
  ...
  return [
    ...
    { label: '业务员', value: d.salesman_name || '-' },
...
const handleSave = async () => {
  ...
  try {
    ...
    let res
    if (form.value.id) {
      res = await updateSalesReturn(form.value.id, data)
    } else {
      res = await createSalesReturn(data)
    }
    ...
    if (res.data?.id) {
      ...
      if (updateCurrentTabPath) {
        updateCurrentTabPath(`/sales-returns?id=${res.data.id}`, `退货订单 ${form.value.code}`)
      }
    }
    loadData()
  } catch (e) {
...
const handleFulfill = async (row) => {
  const id = row?.id || form.value?.id
  if (!id) return
  await ElMessageBox.confirm('确认退货入库？将自动更新库存', '退货确认', { type: 'warning' })
  try {
    await fulfillSalesReturn(id)
    form.value.status = 2
...
const handleCopy = () => {
  ...
  router.push({ path: '/sales-returns', query: { id: 'copy-' + ts } })
}

const handleNew = () => {
  router.push({ path: '/sales-returns', query: { id: 'new-' + Date.now() } })
}

const handleReverse = async (reason) => {
  try {
    await reverseSalesReturn(form.value.id, { reason })
    form.value.status = 3
...
watch(() => route.query.id, (newId) => {
  if (route.path === '/sales-returns') loadOrder(newId)
})
