    ...
    <el-card class="header-card" style="position:relative">
      ...
      <div class="page-header">
        <div class="page-title">
          <span class="title-text">退货单</span>
        </div>
...
<script setup>
defineOptions({ name: 'ReturnDeliveries' })
import { ref, computed, onMounted, onActivated, watch, inject } from 'vue'
import { useRoute, useRouter } from 'vue-router'
import { ElMessage, ElMessageBox } from 'element-plus'
import { getReturnDeliveries, createReturnDelivery, updateReturnDelivery, getReturnDelivery, warehouseConfirmReturn, financeConfirmReturn, reverseReturnDelivery, getCustomers, getWarehouses, getSalesmen, getEmployees } from '../../api'
...
const handleSave = async () => {
  ...
  try {
    ...
    const res = form.value.id ? await updateReturnDelivery(form.value.id, data) : await createReturnDelivery(data)
    ...
    if (res.data?.id) {
      ...
      if (updateCurrentTabPath) {
        updateCurrentTabPath(`/return-deliveries?id=${res.data.id}`, `退货单 ${form.value.code}`)
      }
    }
  } catch (e) {
...
const handleAudit = async () => {
  ...
  await ElMessageBox.confirm('确认审核该退货单？', '审核确认', { type: 'warning' })
  await warehouseConfirmReturn(form.value.id)
  form.value.status = 2
...
const handleCopy = () => {
  ...
  router.push({ path: '/return-deliveries', query: { id: 'copy-' + ts } })
}

const handleNew = () => {
  router.push({ path: '/return-deliveries', query: { id: 'new-' + Date.now() } })
}

const handleReverse = async (reason) => {
  try {
    await reverseReturnDelivery(form.value.id, { reason })
    form.value.status = 3
...
const loadOrder = async (id) => {
  ...
  try {
    const res = await getReturnDelivery(id)
...
watch(() => route.query.id, (newId) => {
  if (route.path === '/return-deliveries') loadOrder(newId)
})
